function rebuild_datasource
    set_color blue
    echo -e "Building Go Datasource Backend project:   Started\033[0K\r"
    set_color white
    set CURRENT (pwd)
    ...
    if docker compose restart datasource-backend >/dev/null
        ...
        while true
            ...
            if [ $started1 -eq 1 ]
                ...
                notify-send --icon "org.gnome.Terminal" --transient $done_phrase
                speak $done_phrase
                break
            else
                set grafana_end (date '+%s')
        ...
        notify-send --icon "org.gnome.Terminal" --transient $err_phrase
        speak $err_phrase
    end
end
